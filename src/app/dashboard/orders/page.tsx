            ...
            <table className="w-full min-w-[800px]">
              <thead className="bg-white/5">
                <tr>
                  ...
                  <th className="text-left p-4 font-bold text-neutral-400 text-xs uppercase">Payment Details</th>
                    ...
                    <tr key={order.id} className={`hover:bg-white/5 transition-colors ${isSelected ? "bg-primary/5" : ""}`}>
                      ...
                      <td className="p-4">
                        <div className="space-y-1">
                          <span className="px-2.5 py-0.5 bg-white/5 border border-white/10 text-neutral-300 rounded-full text-[10px] font-bold uppercase">
                            {order.payment_method || "COD"}
                          </span>
                          {order.payment_number && (
                            <p className="text-xs text-neutral-400 font-mono">From: {order.payment_number}</p>
                          )}
                          {order.transaction_id && (
                            <p className="text-xs text-primary font-mono font-bold">TrxID: {order.transaction_id}</p>
                          )}
                        </div>
                      </td>
                      <td className="p-4">
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="bg-neutral-800/80 p-4 rounded-xl space-y-2 text-sm">
                ...
                {selectedOrder.payment_proof_url && (
                  <div className="pt-2">
                    <p className="text-neutral-400 font-bold uppercase text-xs mb-1">Payment Proof Screenshot:</p>
                    <a
                    ...
                    >
                      <ExternalLink size={14} /> View Screenshot Image
                    </a>
                  </div>
                )}
              </div>
      ...
      {editingOrder && (
        <Dialog open={!!editingOrder} onOpenChange={() => setEditingOrder(null)}>
          <DialogContent className="bg-neutral-900 border-white/10 text-white max-w-md">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold uppercase">Edit Order #{editingOrder.id}</DialogTitle>
            </DialogHeader>
